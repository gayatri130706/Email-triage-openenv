...
@app.get("/")
def root():
    return {"status": "Email Triage API running"}


@app.get("/health")
def health():
    # Must return 200 OK for validator
    return {"status": "ok"}
...
def reset(req: ResetRequest = ResetRequest()) -> ResetResult:
    ...
    if req.task not in TASKS:
        raise HTTPException(
            status_code=400,
            detail=f"Unknown task '{req.task}'. Valid: {list(TASKS.keys())}"
        )
    try:
        _env = EmailTriageEnv()
...
def main():
    ...
    uvicorn.run(
        ...
        host="0.0.0.0",
        port=7860,
        reload=False  # Disable reload for container/submission
    )
